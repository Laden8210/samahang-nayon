<div class="w-full h-screen shadow-lg rounded-lg overflow-hidden z-0">
    <div class="grid grid-cols-12 h-full">
                    ...
                    <div class="relative p-3 bg-white rounded-lg shadow-sm cursor-pointer hover:bg-gray-200">
                        <button class="flex flex-col w-full text-left" wire:click="selectGuest({{ $guest->GuestId }})">
                            <div class="flex justify-between items-center w-full">
                                ...
                                <!-- Unread notification -->
                                @if ($guest->messages->where('IsReadEmployee', false)->isNotEmpty())
                                    <span class="bg-red-500 text-white text-xs font-bold rounded-full px-2 py-1 ml-2">New</span>
                                @endif
                            </div>
                        </button>
                    </div>
        ...
        <div class="col-span-9 bg-white flex flex-col h-full">
            ...
            <div class="flex-1 overflow-y-auto p-4" wire:poll.debounce.1000ms style="background-color: #f0f2f5; max-height: calc(100vh - 160px);">
                @if ($selectedGuest)
                    @foreach ($selectedGuest->messages as $message)
                        <div class="mb-3 flex {{ $message->isGuestMessage === 0 ? 'justify-end' : 'justify-start' }}">
                            <div class="rounded-lg p-3 max-w-xs {{ $message->isGuestMessage === 1 ? 'bg-white shadow' : 'bg-blue-500 text-white' }}">
                                <p class="text-sm">{{ $message->Message }}</p>
                                <span class="text-xs {{ $message->isGuestMessage === 1 ? 'text-gray-500' : 'text-blue-100' }}">
            ...
            <div class="border-t border-gray-300 p-4 bg-gray-100 sticky bottom-0">
                <form wire:submit.prevent="sendMessage" class="flex items-center gap-2">
                    <textarea class="w-full p-3 border border-gray-300 rounded-full resize-none focus:outline-none focus:ring-2 focus:ring-blue-400"
                        rows="1" wire:model="message" placeholder="Type a message..."></textarea>
                    <button type="submit" class="bg-blue-500 text-white px-5 py-2 rounded-full font-semibold">Send</button>
                </form>
            </div>
        </div>
    </div>
